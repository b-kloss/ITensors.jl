...
@inline function lmul!(G::DiagRotation, A::AbstractVecOrMat)
...
@inline function rmul!(A::AbstractMatrix, G::DiagRotation)
    Base.require_one_based_indexing(A)
    m, n = size(A, 1), size(A, 2)
    if G.i2 > n
        throw(DimensionMismatch("column indices for rotation are outside the matrix"))
  ...
  struct BCSRotation{T} <: LinearAlgebra.AbstractRotation{T} 
    rotations::Tuple{Givens{T},Givens{T},DiagRotation{T},DiagRotation{T}}
  end
  struct BCSCircuit{T} <: LinearAlgebra.AbstractRotation{T}
    rotations::Vector{BCSRotation{T}}
  end
  ...
  function LinearAlgebra.lmul!(R::Union{Circuit,BCSRotation,BCSCircuit}, A::AbstractArray)
  ...
  function LinearAlgebra.rmul!(A::AbstractMatrix, adjR::Adjoint{<:Any,<:Union{Circuit,BCSRotation,BCSCircuit}})
    R = adjR.parent
    @inbounds for i in 1:length(R.rotations)
  ...
  Base.:*(g1::BCSCircuit, g2::BCSCircuit) = BCSCircuit(vcat(g2.rotations, g1.rotations))
  LinearAlgebra.lmul!(g1::Circuit, g2::Circuit) = append!(g2.rotations, g1.rotations)
  LinearAlgebra.lmul!(g1::BCSCircuit, g2::BCSCircuit) = append!(g2.rotations, g1.rotations)
  ...
  function givens_rotations(v::AbstractVector{ElT}) where {ElT}
    ...
    for n in reverse(1:(N - 1))
      g, r = givens(v, n, n + 1)
      v = g * vBase.require_one_based_indexing
    return gs, r
  end
  
  function generalized_rotations(v0::AbstractVector{ElT}) where {ElT}
    N = div(length(v0),2)
    gs = Circuit{ElT}([])
    v=copy(v0)
    r = v[2]
    for n in reverse(1:(N-1))
      ...
      v=d1*v
      v=d2*v
      g1, r = givens(convert.(Elt,abs.(v)), 2*n, 2*(n + 1))   ###Real Givens rotation only here, perform on absolute value for example
      c,s=g.c,g.s
      @assert isclose(imag(s),0)
      @assert isclose(imag(c),0)
    ...
    for n in reverse(2:(N-1))
      ###figure out what is the right order for proper conversion to many-body gates/matching conventions
      d2 = diagrotation(v[2*n-1],v[2*n-1],2*n-1,2*n+1)
      d1 = DiagRotation(2*n, 2*(n+1),conj(d1.d))  ##to respect convention with respect to the definition of the Angle θ, naming swapped
      v=d1*v
      v=d2*v
      g1, r = givens(convert.(Elt,abs.(v)), 2*n-1, 2*n+1)   ##here naming/order doesn't matter since no sign change in the angle
      c,s=g.c,g.s
      @assert isclose(imag(s),0)
      @assert isclose(imag(c),0)
    ...
    d1=DiagRotation(1,4,conj(d1.d)) ###make this d1 for convention of sign of angle for later conversion to mb gates
  ...
  function correlation_matrix_to_gmps(
    Λ0::AbstractMatrix{ElT}; eigval_cutoff::Float64=1e-8, maxblocksize::Int=size(Λ0, 1),is_bcs::Bool=false,
  ) where {ElT<:Number}
    Λ = Hermitian(Λ0)
    N = size(Λ, 1)
    V = Circuit{ElT}([])
    ...
    for i in 1:div(N,factor)
      ...
      for blocksize in 1:maxblocksize
        ...
        ΛB = @view Λ[factor*i:j, factor*i:j]
        nB, uB = eigen(Hermitian(ΛB))
        if is_bcs
          p=sortperm(nB)
          n1 = nB[p[1]]  #
          n2 = nB[p[-1]]
          @assert isclose(n1,1-n2)
          err=n1
          n=n1
  ...
  function correlation_matrix_to_mps(
    ...
    elseif 2*length(s)==size(Λ, 1)
      ...
      ###interlace s with a copy of s if bcs state in a different variable doubled_s
    if is_bcs==true
      doubled_s=interleave(s,copy(s))
    end
    ns, C = correlation_matrix_to_gmps(
      Λ; eigval_cutoff=eigval_cutoff, maxblocksize=maxblocksize,is_bcs=is_bcs
    )   ###bcs or not should trigger different behaviour
    if all(hastags("Fermion"), s)
      U = [ITensor(is_bcs ? doubled_s : s, g) for g in reverse(C.rotations[begin:2:end])] ###here we could simply iterate C.rotations[::2] for bcs
      is_bcs && U=U[begin:2:end].*U[begin+1:2:end]  ###condense diagonal and real givens into same gate
      #and we might want to make use of the fact that U for bcs as defined above can be concatenated (adjacent diag+givens have same support and can be condensed)
      ψ = MPS(s, n -> round(Int, ns[n]) + 1, U; kwargs...)      ##should in principle work for bcs right away, if corr_to_gmps was adapted properly (i.e. returning the vaccuum only?)
      ##check that the correct vaccuum state is used
